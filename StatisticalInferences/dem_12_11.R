...
dem_12_11_matrix <- matrix(
    data = c(
        ...
        2.19, 1.86, 0.75, 1.24, 0.72, 9.80,
        1.73, 1.67, 0.64, 1.14, 0.55, 6.88
    ), nrow = 22, ncol = 6, byrow = TRUE
)

dem_12_11 <- dem_12_11_matrix %>% as.data.frame()
dem_12_11_colnames <- c(
    "RostalLength", # x1, in inches
    "WingLength", # x2, in inches
    "RostalToNotchLength", # x3, in inches
    "NotchToWingLength", # x4, in inches
    "Width", # x5, in inches
    "Weight" # y, in pounds
)
names(dem_12_11) <- dem_12_11_colnames
View(dem_12_11)
write.csv(dem_12_11, file = "datasets/dem_12_11.csv")
summary(dem_12_11)

# construct the multiple linear regression model
dem_12_11_lm <- lm(
    formula = Weight~RostalLength +
        WingLength +
        RostalToNotchLength+
        NotchToWingLength+
        Width,
